...
def get_img_id(path, reader):
    ...
    if args.rotate == -1:
        file = RotateAntiClockWise90(one_channel)
    elif args.rotate == 1:
        file = RotateClockWise90(one_channel)
...
def locate_grid(img, filename, eng):
    scale_time = double(eng.cal_reddot(matlab.uint8(img.tolist()), 0))
...
def main(margs, threadsnum=3):
    reader = easyocr.Reader(['en'])
    imgs = glob.glob(f'{margs.imgs_path}/IMG_3100.jpeg')
    mididx = int(len(imgs) / threadsnum) + 1

    ts = []
    for i in range(threadsnum):
        t1 = threading.Thread(target=read_thread, args=(margs, imgs[i * mididx: (i+1)*mididx],reader,), daemon=True)
        t1.start()
        ts.append(t1)
